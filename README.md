...
Conditional logit with constructed choice sets is the standard approach for estimating partner preferences from observed unions. It treats each individual as choosing a partner from a set of alternatives, separating preference from opportunity. The approach is powerful, but it rests on assumptions that are well-known yet rarely tested quantitatively.

These simulations generate partner markets under known data-generating processes (DGPs), estimate clogit models, and check whether the true parameters are recovered. Five modules test specific threats to identification:
